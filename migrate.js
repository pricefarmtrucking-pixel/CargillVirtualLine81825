// migrate.js — CommonJS, idempotent
// Usage: `node migrate.js`  (Render build step can call `npm run migrate`)
...
function applySchema() {
  ...
  // ---------- Fallback base schema (safe to run multiple times) ----------
  const base = `
...
CREATE TABLE IF NOT EXISTS slot_reservations (
  ...
  est_unit TEXT,
  manage_token TEXT,
  queue_code TEXT,                -- 4‑digit Probe Code (added/ensured below if missing)
  created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
function ensureProbeCodeColumn() {
  ...
  exec(
    ...
    'Ensured probe index (idx_resv_probe)'
  );
}

function ensureIndexes() {
  exec(
    `CREATE INDEX IF NOT EXISTS idx_slots
       ON time_slots (site_id, date, slot_time);`,
    'Ensured idx_slots on time_slots'
  );
  exec(
    `CREATE INDEX IF NOT EXISTS idx_resv_lookup
       ON slot_reservations (site_id, date, slot_time);`,
    'Ensured idx_resv_lookup on slot_reservations'
  );
  exec(
    `CREATE INDEX IF NOT EXISTS idx_otp_phone_active
       ON otp_codes (phone, expires_at, consumed_at);`,
    'Ensured idx_otp_phone_active on otp_codes'
  );
}
...
function main() {
  ...
  ensureProbeCodeColumn();  // add queue_code + index if missing
  ensureIndexes();          // general performance indexes
