...
// ---- basic env loader (reads server/.env if present) ----
const ENV = {};
try {
...
// ---- Twilio setup ----
const TWILIO_SID  = process.env.TWILIO_SID  || ENV.TWILIO_SID;
const TWILIO_AUTH = process.env.TWILIO_AUTH_TOKEN || ENV.TWILIO_AUTH_TOKEN;
const TWILIO_FROM = process.env.TWILIO_PHONE_NUMBER || ENV.TWILIO_PHONE_NUMBER;
const smsClient = (TWILIO_SID && TWILIO_AUTH) ? twilio(TWILIO_SID, TWILIO_AUTH) : null;
...
async function sendSMS(to, body) {
  ...
  if (!to) {
    console.warn('SMS skipped: missing recipient phone');
    return { sent: false, reason: 'missing_to' };
  }
  try {
    const msg = await smsClient.messages.create({ from: TWILIO_FROM, to, body });
...
function fmtSlot(siteId, date, time) {
  return `Site ${siteId} on ${date} at ${time}`;
}

// ---- Express + CORS + cookies ----
app.set('trust proxy', 1); // needed for Secure cookies behind Render’s proxy
app.use(express.json());
app.use(cookieParser(ENV.SESSION_SECRET || 'secret'));

// Allow one or more frontend origins (comma-separated in env), trim trailing slash
const ORIGINS = (process.env.CORS_ORIGIN || ENV.CORS_ORIGIN || '')
  .split(',')
  .map(s => s.trim().replace(/\/$/, ''))
  .filter(Boolean);

// If ORIGINS empty, allow all (useful while testing). Otherwise, use the list.
const corsOrigins = ORIGINS.length ? ORIGINS : true;
...
// serve static client (same origin UI)
app.use(express.static(path.join(__dirname, 'public')));

// ---- helpers ----
function normalizePhone(p) {
  const d = String(p || '').replace(/\D/g, ''); // digits only
...
function sessionCookieOptions() {
  return {
    httpOnly: true,
    secure: true,            // Render is HTTPS; required for SameSite=None
    sameSite: 'none',        // works for cross-site requests with credentials: 'include'
...
app.get('/api/whoami', (req, res) => res.json({ session: !!req.cookies?.session }));

// ---------- Auth (OTP) ----------
app.post('/auth/request-code', async (req, res) => {
  const phone = normalizePhone(req.body?.phone);
  if (!phone) return res.status(400).json({ error: 'invalid phone; use 10-digit US number' });
...
app.post('/api/slots/cancel', async (req, res) => {
  ...
  const to = normalizePhone(r.driver_phone);
  const slotLabel = fmtSlot(r.site_id, r.date, r.slot_time);
  ...
  const body = `Your appointment at ${slotLabel} was CANCELED${reason ? `: ${reason}` : ''}.`;
...
app.post('/api/slots/mass-cancel', async (req, res) => {
  ...
  for (const id of reservation_ids) {
    const r = db.prepare("SELECT id, driver_phone, slot_time FROM slot_reservations WHERE id=? AND site_id=? AND date=?").get(id, site_id, date);
    if (r) victims.push(r);
  }

  const tx = db.transaction(() => {
    for (const v of victims) {
      db.prepare("UPDATE time_slots SET reserved_truck_id=NULL, reserved_at=NULL WHERE site_id=? AND date=? AND slot_time=? AND reserved_truck_id=?").run(site_id, date, v.slot_time, v.id);
      db.prepare("DELETE FROM slot_reservations WHERE id=?").run(v.id);
    }
  });
  tx();
  ...
  if (notify) {
    notified = [];
    for (const v of victims) {
      const to = normalizePhone(v.driver_phone);
      const body = `Your appointment at ${fmtSlot(site_id, date, v.slot_time)} was CANCELED${reason ? `: ${reason}` : ''}.`;
      /* eslint-disable no-await-in-loop */
      const sms = await sendSMS(to, body);
      notified.push({ id: v.id, slot_time: v.slot_time, ...sms });
    }
  }

  res.json({ ok: true, canceled: victims.length, notified });
});

// ---------- Notify only ----------
app.post('/api/slots/notify', async (req, res) => {
  const { reservation_id, message } = req.body || {};
  if (!reservation_id || !message) return res.status(400).json({ error: 'reservation_id and message required' });

  const r = db.prepare("SELECT site_id, date, slot_time, driver_phone FROM slot_reservations WHERE id=?").get(reservation_id);
  if (!r) return res.status(404).json({ error: 'not found' });

  const to = normalizePhone(r.driver_phone);
  const body = `${message} (${fmtSlot(r.site_id, r.date, r.slot_time)})`;
  const sms = await sendSMS(to, body);
